...
class NewmanGreedy:
    ...
    def __init__(self, graph, snapshot_size=None, forced_clusters = None):
        ...
        self.run_greedy_clustering()
        self.join_separate_clusters()
        
        nx.relabel_nodes(self.dendrogram, self.rename_map.original, copy=False)

    def join_separate_clusters(self):
        if len(self.super_graph) > 1:
            print len(self.super_graph)
            for node in self.super_graph:
                if node != max(self.super_graph):
                    self.super_graph.add_edge(node, max(self.super_graph))
                    self.super_graph[node][max(self.super_graph)] = 0
                    self.super_graph[max(self.super_graph)][node] = 0
            self.reheapify()
            self.run_greedy_clustering()
    ...
    def run_greedy_clustering(self, reheap_steps=500):
        ...
        for num_clusters in xrange(self.super_graph.number_of_nodes(), 1, -1):
            ...
            if sum([id1, id2, qual_diff]):
                self.combine_clusters(id1, id2)
                self.quality += qual_diff
